    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants
          .map(email => `<li data-email="${email}" data-activity="${name}"><span>${email}</span><button class="delete-btn" type="button" title="Remove participant">✕</button></li>`)
          .join('');
        ...
        activityCard.querySelectorAll('.delete-btn').forEach(btn => {
          btn.addEventListener('click', async (e) => {
            e.preventDefault();
            const email = btn.closest('li').dataset.email;
            const activity = btn.closest('li').dataset.activity;
